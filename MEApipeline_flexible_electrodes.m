...
rawData = '/Users/timothysit/flexible-electrodes/data/mat_data/merged';  % path to raw data .mat files
Params.outputDataFolder = nan;  % specify the main folder to save output data to, set to nan to be same as HomeDir
Params.priorAnalysisPath = '/Users/timothysit/AnalysisPipeline/OutputData17Jan2023';  % path to prev analysis
spikeDetectedData = '/Users/timothysit/AnalysisPipeline/OutputData17Jan2023'; % path to spike-detected data

% Input and output filetype
spreadsheet_file_type = 'csv'; % 'csv' or 'excel'
spreadsheet_filename = 'flexible_electrodes_merged.csv'; 
...
Params.priorAnalysisDate = '17Jan2023'; % prior analysis date in format given in output data folder e.g., '27Sep2021'
...
Params.fs = 20000; % Sampling frequency, HPC: 25000, Axion: 12500;
Params.dSampF = 20000; % down sampling factor for spike detection check
Params.potentialDifferenceUnit = 'uV';  % the unit which you are recording electrical signals % TODO: double check for flexible electrodes...
...
if ((Params.priorAnalysis == 0) || (Params.runSpikeCheckOnPrevSpikeData)) && (Params.startAnalysisStep == 1) 
    ...
    for  ExN = 1:length(ExpName)
        ...
        % temp modification 
        Info.rawData = '/Users/timothysit/flexible-electrodes/data/mat_data/merged';
...
if any(strcmp(Params.optionalStepsToRun,'runStats'))
    ...
    % Determine which classification mode to use
    numGrps = length(unique(recordingLevelData.Grp));
    numDIVs = length(unique(recordingLevelData.DIV));

    if (numGrps > 1) && (numDIVs > 1)
        Params.classificationMode = 'genotypePerDIV';
    elseif numGrps > 1
        Params.classificationMode = 'genotype';
    elseif numDIVs > 1
        Params.classificationMode = 'DIV';
    else 
        Params.classificationMode = 'NULL';
    end 
    ...
    Params.clf_num_kfold_repeat = 1;
    Params.clf_num_kfold = 5;
    Params.doPairwiseClassification = 1;
    Params.scaleClfPerformanceWbaseline = 0;  % not needed if Params.downsampleMajorityClass = 1
    Params.downsampleMajorityClass = 1;
    Params.numDownSampleRuns = 10;
    Params.doClfShuffleControls = 0;
    Params.numClfShuffleControls = 10;

    if ~strcmp(Params.classificationMode, 'NULL')
        ...
        for lag_val = Params.FuncConLagval
            ...
            LDAresults = doLDA(recordingLevelData, Params, lag_val);
            plotLDAresults(LDAresults, Params, LDAresults.classificationMode, plotSaveFolder);

            if strcmp(Params.classificationMode, 'genotypePerDIV')
                classificationResults = classifyGroupPerDIV(recordingLevelData, Params, lag_val);
            else
                classificationResults = doClassification(recordingLevelData, Params, lag_val);
            end 

            plotClassificationResults(classificationResults, Params, plotSaveFolder)

        end 
        
    else 

        fprintf('There are no unique groups or DIVs, skipping classification step \n')
