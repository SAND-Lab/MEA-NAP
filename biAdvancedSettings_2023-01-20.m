...
Params.maxNumSpikeWaveformsToPlot = 50;
...
Params.unitLevelNetMetLabels = {'node degree','mean edge weight','node strength', ... 
...
elseif strcmp(Params.channelLayout, 'flexibleElectrodes')

    % Get the electrode layout specified 
    num_channels = 32;
    channelNames = [0, 31, 1, 30, 2, 29, 3, 28, 4, 27, 5, 26, 6, 25, 7, 24, 8, ...
        22, 9, 21, 10, 20, 11, 19, 12, 18, 13, 17, 14, 16, 15, 23];
    
    Params.channels = channelNames;
    xcoords = zeros(32, 1);
    xcoords(1:2:end) = repelem(linspace(1, 0.05, 8), 1, 2);
    xcoords(2:2:end) =  repelem(linspace(1, 0.05, 8), 1, 2) - 0.05;
    ycoords = linspace(0, 1, 32)';
    Params.coords = [xcoords, ycoords] * 8;
