...
rawData = '/Volumes/Macintosh HD/Users/timothysit/flexible-electrodes/data/mat_data';  % path to raw data .mat files
Params.priorAnalysisPath = [];  % path to prev analysis
spikeDetectedData = ''; % path to spike-detected data

% Input and output filetype
spreadsheet_file_type = 'csv'; % 'csv' or 'excel'
spreadsheet_filename = 'flexible_electrodes.csv'; 
...
Params.optionalStepsToRun = {'generateCSV'}; % include 'generateCSV' to generate csv for rawData folder
...
if ((Params.priorAnalysis == 0) || (Params.runSpikeCheckOnPrevSpikeData)) && (Params.startAnalysisStep == 1) 
    ...
    for  ExN = 1:length(ExpName)
        ...
        if Params.runSpikeCheckOnPrevSpikeData
            spikeDetectedDataOutputFolder = fullfile(spikeDetectedData, ...
                '1_SpikeDetection', '1A_SpikeDetectedData');
        else
...
fprintf('Running step 3 of MEA-NAP: functional connectivity \n')
...
fprintf('Running step 4 of MEA-NAP: network activity \n')
...
if Params.priorAnalysis==0 || Params.priorAnalysis==1 && Params.startAnalysisStep<=4
    ...
    NodeCartographyByLagPlotFolder = fullfile(Params.outputDataFolder, ...
    strcat('OutputData',Params.Date), '4_NetworkActivity', ...
    '4B_GroupComparisons', '6_NodeCartographyByLag');

    plotNetMetNodeCartography(combinedData, ExpName,Params,HomeDir, NodeCartographyByLagPlotFolder)
