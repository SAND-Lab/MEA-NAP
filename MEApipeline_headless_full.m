function MEApipeline_headless_full(InputParamsFilePath)
...
%
% =====================================================================
% HEADLESS (Params.guiMode=0) copy of MEApipeline.m for running the FULL
% pipeline (Step 1 spike detection -> Step 2 neuronal activity -> Step 3
% functional connectivity -> Step 4 network activity) on raw, not-yet-spike-
% detected recordings. Use this when you have original raw MEA recordings;
% use MEApipeline_headless_fromStep2.m instead if you already have spike-
% detected data and just want Steps 2-4.
%
% Run via `matlab -batch "MEApipeline_headless_full"` (see run_meanap.sh for
% cluster submission). Only the "Directories"/settings section below (up to
% "Params.guiMode = 0") is meant to be edited per dataset — the rest of the
% file is a faithful copy of MEApipeline.m, plus the fixes noted below.
%
% Real bugs in stock MEA-NAP found and worked around (all headless/non-GUI-
% execution issues -- see MEApipeline_headless_fromStep2.m's header for the
% full list and explanations; the ones relevant to a full Step-1-4 run):
%   - batchDetectSpikes(...) needs a 6th arg in the non-GUI branch.
%   - Params.electrodesToGroundPerRecording needs to be a cell array.
%   - AdvancedSettings.m's default netMetToCal includes dead-code 'PC_raw'.
%   - Several Params fields are GUI-only with no AdvancedSettings.m fallback.
%   - AdvancedSettings.m resets Params.timeProcesses; re-applied below.
%   - PlotIndvNetMet.m's combined-plot copyobj() can throw under headless
%     software OpenGL; Functions/PlotIndvNetMet.m now catches this.
%
...
%      struct() — see the call site below.
%   2. Params.electrodesToGroundPerRecording is left as a plain empty
%      double `[]` by setUpSpreadSheet.m when the recording spreadsheet
%      has no 'Ground' column (true for ExampleData/exampleData.csv), but
%      MEApipeline.m's Step 2/3/4 loops unconditionally brace-index it
%      (`{ExN}`), which only works on a cell array. Worked around by
%      converting it to a cell array of empty strings after
%      setUpSpreadSheet runs.
%   3. AdvancedSettings.m's default Params.netMetToCal includes 'PC_raw',
%      but ExtractNetMet.m's actual PC_raw computation is dead code
%      (wrapped in a commented-out fcn_find_hubs_wu block) — crashes
%      Step 4's final eval-assignment loop with "Unrecognized function or
%      variable 'PC_raw'". Worked around by overriding Params.netMetToCal
%      to the real list a completed GUI run actually uses (extracted from
%      Parameters_OutputData26Jun2025.csv, which — critically — was
%      generated by a GUI run, so its netMetToCal reflects
%      getParamsFromApp.m's filtering, not AdvancedSettings.m's raw
%      default).
...
Params.outputDataFolderName = 'OutputDataExampleRun1'; % EDIT: name for this run's output subfolder
rawData = '/imaging/astle/alex/MEA-NAP-storage/ExampleData'; % EDIT: folder of raw recording .mat files
Params.priorAnalysisPath = [''];
spikeDetectedData = '';

% Input and output filetype
spreadsheet_file_type = 'csv'; % 'csv' or 'excel'
spreadsheet_filename = '/imaging/astle/alex/MEA-NAP-storage/ExampleData/exampleData_subset.csv'; % EDIT: batch CSV
...
% AdvancedSettings.m's default Params.netMetToCal includes 'PC_raw', but
% ExtractNetMet.m's actual PC_raw computation is dead code (wrapped in a
% commented-out fcn_find_hubs_wu block) — so the AdvancedSettings.m default
% crashes Step 4 with "Unrecognized function or variable 'PC_raw'" the
% moment ExtractNetMet.m's final eval-assignment loop reaches it. This is a
% genuine bug in MATLAB MEA-NAP's headless/non-GUI path: getParamsFromApp.m
% (the GUI) builds its OWN netMetToCal that silently excludes 'PC_raw' (and
% 'Cmcblty'/'NE'), so GUI users never hit this. Overridden here to the real
% list extracted from Parameters_OutputData26Jun2025.csv, a completed
% GUI-driven run on this exact dataset — this is "what MEA-NAP actually
% computes in practice", not a guess.
