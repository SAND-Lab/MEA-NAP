...
The automatic threshold is the valley between the two modes of the ISI\ :sub:`N` distribution: a short-interval mode produced by spikes inside bursts, and a longer-interval mode produced by the array's background firing. On a densely firing array the two modes sit close together — a 16-electrode recording firing at 460 Hz array-wide has its background mode near 20 ms — so the threshold that separates them can be a few milliseconds rather than the ~100 ms typical of a sparsely firing culture.

At those short thresholds a single network event no longer comes out of the detector as a single burst. ISI\ :sub:`N` detection splits on the interval between individual spikes, and through the decaying tail of an event the background firing keeps ISI\ :sub:`N` crossing back and forth over the threshold, so the detector ends one burst and starts another each time. A 400 ms network event can be reported as twenty bursts a few milliseconds apart, which leaves the burst count, burst rate and burst duration describing fragments rather than events.

``Params.bakkumNetworkBurstMergeGapMs`` (default 20 ms) rejoins them: network bursts less than that far apart are reported as one burst. Merging only re-segments bursts the detector already found — it cannot add time or spikes that were detected as quiet — so it changes burst count, rate and duration while leaving the fraction of the recording and the fraction of spikes in bursts essentially unchanged. Set it to 0 to report every fragment separately, as versions before this one did.
