...
1. Upload the HPC version of MEA-NAP to your remote server:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
1. Upload HPC version of MEA-NAP directly to your remote server:
   ...
   - Verify that the following lines which indicate the successful download of HPC version of MEA-NAP appear.
...
2. Download HPC version of MEA-NAP locally and upload to your remote server:
   ...
   - Download HPC version of MEA-NAP `here <https://github.com/SAND-Lab/MEA-NAP-HPC>`__.
   
   - Open a terminal window.

   .. _HPC_step1_remote_directory:
   ...
   - After successfully running the command, SSH into your remote server.
...
2. Change your current directory to the MEA-NAP directory on your remote server:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
3. Create a directory for mat files in the MEA-NAP directory on your remote server:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
4. Upload mat files to your remote server:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
1. Upload one local mat file to your remote server:
   ...
   - ``mat_file_dir``: path of your remote server directory where you store your mat files (:ref:`see step 3 <HPC_step3>`)
...
2. Upload multiple local mat files to your remote server:
   ...
   - ``mat_file1 mat_file2 mat_file3 …``: paths of mat files on your local device. **These file paths should be separated by spaces.**
   - ``mat_file_dir``: path of your remote server directory where you store your mat files (:ref:`see step 3 <HPC_step3>`)
...
5. Upload CSV file(s) to the MEA-NAP directory to your remote server:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
- **For these methods, open another terminal window that is not logged into your remote server**.
  ...
  2. Upload multiple CSV files: 
     ...
     - ``/remote/directory``: path of the MEA-NAP directory on your remote server (:ref:`see step 1 <HPC_step1>`)
...
6. Change the working directory to the MEA-NAP directory on your remote server:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
7. Modify MEApipeline.m
^^^^^^^^^^^^^^^^^^^^^^^^^^

There are 3 **ways** to edit MEApipeline.m.

For all methods, edit MEApipeline.m according to this `documentation <https://analysis-pipeline.readthedocs.io/en/latest/pipeline-steps.html>`_.
...
8. Create a bash script to submit jobs for MEApipeline.m:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
- Use this example bash script for reference.
  ...
  - ``/remote/directory``: path of the MEA-NAP directory on your remote server (:ref:`see step 1 above <HPC_step1>`)
...
9. Submit a job with your bash script:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
10. Check log files to view progress and error messages:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
11. Download MEA-NAP outputs from your remote server:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
