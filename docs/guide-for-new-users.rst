...
- **MEA Data Folder:** Location of the folder with **your raw data.** All of the recordings that you want to analyze in one experiment should be in the same folder. The MATLAB pipeline needs the data in .mat format — see Setting up MEA-NAP for instructions on using the GUI's File Conversion tab. The Python pipeline additionally reads Multichannel Systems .h5 and Axion .raw files directly, with no conversion step; see :doc:`python/gui-guide` for the settings each format needs.
