function [PC_norm, PC_norm_original, PC_residual,PC,between_mod_k] = participation_coef_norm_tim(W,Ci,n_iter,par_comp)
...
if par_comp == 0 % no parallel computing
    ...
    k_m_rand = zeros(n_iter, max(Ci), n);
    ...
    for ii = 1:n_iter % number of randomizations
        ...
        % W : the original matrix 
        k_m_rand_iter_ii = zeros(max(Ci), n);
        ...
        for iii = 1:max(Ci) % Below we estimate the squared difference between original and randomised intramodular degree, for each module
            ...
            % Original calculation
            Kc2_rnd_loop = Kc2_rnd_loop+(((sum(W.*(Gc==iii),2)./Ko)-(sum(W_rnd.*(Gc_rnd==iii),2)./Ko)).^2);
            k_m_rand_iter_ii(iii, :) = sum(W_rnd.*(Gc_rnd==iii),2);
        end
...
% Tim 2022-11-06 : changed to using the mean because mean is linear, and
% median is not so the above "trick" may not work as intended... 
PC_norm_original = ones(n,1) - mean(Kc2_rnd,2);
PC_norm_original(~Ko) = 0;

% Taking the median outside 
b0 = 0.5;
k_m_rand_median = squeeze(median(k_m_rand, 1));
Ko_tiled = repmat(Ko, 1, max(Ci))';
squared_diff = ((k_m - k_m_rand_median) ./ Ko_tiled) .^2; 
squared_diff_summed = sum(squared_diff, 1)';  % sum together communities m
PC_norm = ones(n, 1) - sqrt(b0 * squared_diff_summed);
