function find_best_spike_result(spike_folder, plot_folder, Params)
...
custom_backup_param_to_use
    if empty, then looks as the grounded electrode instead
    lost param to use in case there is no TTX file
Outputs 
---------------
figures : (.png)
...
2021-04-15 Tim Sit : created file
2022-01-04 Tim Sit : adapting the script into a function 
%}

%% Define file to find best params, and parameters 
raw_data_folder = '';
if not(isfolder(spike_folder))
    fprintf('Spike detection result folder is not a folder, please double check your entry \n')
end 
...
max_tolerable_spikes_in_TTX_abs = Params.prePostTTX.max_tolerable_spikes_in_TTX_abs; 
max_tolerable_spikes_in_grounded_abs = Params.prePostTTX.max_tolerable_spikes_in_grounded_abs;
max_tolerable_spikes_in_TTX_per_s = Params.prePostTTX.max_tolerable_spikes_in_TTX_per_s; 
max_tolerable_spikes_in_grounded_per_s = Params.prePostTTX.max_tolerable_spikes_in_grounded_per_s;
start_time = Params.prePostTTX.start_time; 
default_end_time = Params.prePostTTX.default_end_time;  
sampling_rate = Params.prePostTTX.sampling_rate;  
threshold_ground_electrode_name = Params.prePostTTX.threshold_ground_electrode_name;
default_grounded_electrode_name = Params.prePostTTX.default_grounded_electrode_name;
min_spike_per_electrode_to_be_active = Params.prePostTTX.min_spike_per_electrode_to_be_active;
wavelet_to_search = Params.prePostTTX.wavelet_to_search;
use_TTX_to_tune_L_param = Params.prePostTTX.use_TTX_to_tune_L_param;
spike_time_unit = Params.prePostTTX.spike_time_unit; 
custom_backup_param_to_use = Params.prePostTTX.custom_backup_param_to_use;  
regularisation_param = Params.prePostTTX.regularisation_param;

%% Loop through each file 

if isempty(raw_data_folder)
    if ~isempty(spike_folder)
        fprintf('No raw data folder specified, getting recording names from wavelet spike results \n');
        recording_names = {dir([spike_folder '*.mat']).name};
...
for n_wavelet = 1:length(wavelet_to_search)
    ...
    for r_name_idx = 1:length(recording_names_exclude_TTX)
        ...
        wavelet_spike_detection_results_names = {dir([spike_folder, strcat(recording_name, '*')]).name};
        ...
        for file_idx = 1:length(wavelet_spike_detection_results_names)
            ...
            param_file = load([spike_folder file_name]);
        ...
        if sum(wavelet_spike_detection_info(:, 3)) == 0
            if ~isempty(custom_backup_param_to_use)
                ...
                wavelet_best_param_file_data = load([spike_folder wavelet_best_param_file_name]);
                ...
                if ~isempty(plot_folder)
                    ...
                    print([plot_folder fig_save_name], '-dpng', '-r300')
                end 
...
writetable(T, [plot_folder 'summary_table.csv']);
