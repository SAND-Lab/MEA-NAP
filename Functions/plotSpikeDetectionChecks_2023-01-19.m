...
for i = 1:length(methods)
    ...
    if ~strcmp(method, 'all')
        ...
        maxNumTimeSamples = 1000;
        if size(spk_waves_method,2) > maxNumTimeSamples
            spk_waves_method = spk_waves_method(:, round(linspace(1,length(spk_waves_method),...
                maxNumTimeSamples)));
        end
        
        numSpikes = size(spk_waves_method, 1);
        if size(spk_waves_method, 1) > Params.maxNumSpikeWaveformsToPlot
            spike_idx = randperm(numSpikes);
            spike_idx_subsampled = spike_idx(1:Params.maxNumSpikeWaveformsToPlot);
            spk_waves_method = spk_waves_method(spike_idx_subsampled, :);
        end 
