function varargout=notBoxPlotRF(y,x,ColCode,MarkerSize,varargin)
...
%                 'linewidth',6);
            if markMedian
                ...
                h(k).med=plot([X(k)-jitScale,X(k)+jitScale],[med(k),med(k)],'color',[0 0 0],...
                    'linewidth',1);
            end
        end
        ...
        h(k).data=plot(thisX, thisY, 'o','MarkerSize',MarkerSize, 'color', ColCode,...
                       'markerfacecolor', ColCode);
    ...
    if strcmp(style,'line')
        for k=1:length(X)
            ...
            h(k).mu=plot(X(k),mu(k),'o','color','b',...
                'markerfacecolor','b',...
                'markersize',10);
            ...
            h(k).sem=plot([X(k),X(k)],[mu(k)-SEM(k),mu(k)+SEM(k)],'-r',...
                'linewidth',2);   
            if markMedian
                h(k).med=plot(X(k),med(k),'s','color',[0 0 1],...
    ...
    function ptch=patchMaker(thisInterval,tColor)
        ...
        ptch=patch([X(k)-jitScale, X(k)+jitScale, X(k)+jitScale, X(k)-jitScale],...
                [l,l,u,u], 0);
        set(ptch,'edgecolor',[0.5 0.5 0.5],'facecolor',[0.8 0.8 0.8])
